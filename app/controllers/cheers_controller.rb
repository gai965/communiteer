class CheersController < ApplicationController
  before_action :move_to_index,       only: [:volunteer_cheer_create]
  before_action :set_cheer_volunteer, only: [:volunteer_cheer_index, :volunteer_cheer_create]
  ...
  def volunteer_cheer_create
    cheer = Cheer.new(cheerable_id: @account.id, cheerable_type:@account_type, targetable_id:@volunteer.id, targetable_type:@target_type)
    if cheer.save!
